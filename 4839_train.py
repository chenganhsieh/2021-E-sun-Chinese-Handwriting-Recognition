...
def macro_f1(y_pred, y_true, classes):
    isnull_idx = in_class_dict['isnull']
    num_ysun_class = len(in_class_dict)
    tps , fps, fns = [0]*num_ysun_class, [0]*num_ysun_class, [0]*num_ysun_class
    num_samples, f1_score = len(y_pred), 0
    for i in range(num_samples):
        y_pred_i = y_pred[i]
        y_true_i = y_true[i]
        y_pred_char = classes[y_pred_i]
        y_true_char = classes[y_true_i]
        if y_pred_i == y_true_i:
            if y_pred_char in in_class_dict: tps[in_class_dict[y_pred_char]] += 1
            else: tps[isnull_idx] += 1
        else:
            if y_pred_char in in_class_dict: fps[in_class_dict[y_pred_char]] += 1
            else: fps[isnull_idx] += 1

            if y_true_char in in_class_dict: fns[in_class_dict[y_true_char]] += 1
            else: fns[isnull_idx] += 1 
    
    for i in range(num_ysun_class):
        if tps[i]:
            tp, fp, fn = tps[i], fps[i], fns[i]
            p ,r = tp/(tp + fp), tp/(tp + fn)
            f1_score += 2*p*r/(p+r)
    acc = np.sum(np.array(y_pred) == np.array(y_true))
    return acc/num_samples, f1_score/num_ysun_class
...
def main(args):
    ...
    for epoch in range(args.num_train_epochs):
        ...
        train_acc, train_f1 = macro_f1(y_pred_list, y_true_list, classes)
        # Validation step
        model.eval()
        y_pred_list , y_true_list= [], []
        with torch.no_grad():
        ...
        eval_acc, eval_f1 = macro_f1(y_pred_list, y_true_list, classes)
